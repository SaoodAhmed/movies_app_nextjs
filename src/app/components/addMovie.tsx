...
const AddMovie: React.FC<{ addMovie: any}> = ({ addMovie }) => {
  ...
  return (
    <div className='mt-6'>
        <form onSubmit={(e)=>handleAdd(e)} className='flex justify-center mx-auto flex-col w-4/12 gap-y-4 ' >
            <Input className=' outline-none text-slate-600 focus:purple-800  focus:border-blue-600' placeholder='Enter image url' value={data.imgSrc} name="imgSrc" onChange={handleOnChange}/>
            ...
            <button type="submit" className="bg-gradient-to-t text-slate-100 w-48 text-lg py-2 mx-auto rounded-md from-purple-800 via-indigo-700 to-blue-500 font-medium">
            Add Movie
            </button>   
        </form>
